...
public class S extends T{
    ...
    public S(List<Target> targets, S oldTask, int timeToRun, int chancesToSucceed, int chancesToBeAWarning, boolean isRandom) {

        super(targets, oldTask, timeToRun, chancesToSucceed, chancesToBeAWarning, isRandom);

        this.cUI = oldTask.getcUI();
    }
    ...
    public void run(){
        ...
        Map<String,List<String>> targetNameToMinionData = new HashMap<>();
    ...
    private Map<String, Minion> startMinionMap(){
        ...
        if(this.minions.isEmpty()) {
            for (Target curTarget : this.targets)
                resM.put(curTarget.getName(), new Minion(curTarget));
        }
        else
        {
